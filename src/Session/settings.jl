function handle_setting!(connection::HTTPConnection, key::Frame.SETTING_IDENTIFIER, value::UInt32)
    ...
    elseif key == Frame.SETTINGS_MAX_CONCURRENT_STREAMS
      connection.settings.max_concurrent_streams = convert(Union{Nothing,typeof(UInt(value))}UInt(value))
    ...
    elseif key == Frame.SETTINGS_MAX_HEADER_LIST_SIZE
      connection.settings.max_header_list_size = convert(Union{Nothing,typeof(UInt(value))}UInt(value))
    else
